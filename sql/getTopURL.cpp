 ...
 *
 * Popularity / Topness of URL is decided by the Number of Views that it has received.
 * Votes also Play a minor part in affecting the ranking BUT don't affect the ordering of
...
int main(int argc, char* args[]) {
	...
	if (argc != 2 ){ // Need 1 Arguments  in this form : [int  number]
		...
		cout << "Usage is getTopURL An_Integer" << endl;
		return (-1);
	}
	...
	if( s >> rank) {
		...
		retv  = db.getTopURL(rank); // print out given urls.
	}
	else {
