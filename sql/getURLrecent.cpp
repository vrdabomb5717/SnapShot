/*
 * getURLrecent.cpp
 * Returns detailed info on the most recent X urls where X is an integer
...
int main(int argc, char* args[]) {
	...
	if (argc != 2 ){ // Need 1 Arguments  in this form : [string id]
		...
		cout << "Usage is getcomment url_id" << endl;
		return (-1);
	}
	...
	if( s >> url_id) { // if we can extract the url id.

		retv  = db.getURLrecent(url_id); // print the Recent Urls
