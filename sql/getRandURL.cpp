/*
 * getRandURL.cpp
 * Return detailed info on Randomly selected URLs from the Table.
 * The number of URLS returned is limited by the number specified at
 * run time.
...
int main(int argc, char* args[]) {
	...
	if (argc != 2 ){ // Need 1 Arguments  in this form : [int number]
		...
		cout << "Usage is getRandURL An_Integer" << endl;
		return (-1);
	}

	string rank_string(args[1]); // get the no of random urls requested.
	...
	int rank; // Number of random URLs to be returned
	int retv; // return value
	if( s >> rank) {

		retv  = db.getRandURL(rank); // print out given urls.
	}
	else {
