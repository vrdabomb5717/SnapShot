/*
 * getBottomURL.cpp
 * Return detailed info on URLs that are the Bottom X of the site where X
 ...
 *
 * UnPopularity / Bottomness of URL is decided by the Number of Views that it has received.
 * Votes also Play a minor part in affecting the ranking. That said, they DO NOT change/alter
 * the ordering of sorting by number of views a URL has received.
 *
...
int main(int argc, char* args[]) {
	...
	if (argc != 2 ){ // Need 1 Arguments  in this form : [string number]
		cout << "-1" <<endl;
		cout << "Insufficient arguments supplied" << endl;
		cout << "Usage is getBottomURL An_Integer" << endl;
		return (-1);
	}
	...
	if( s >> rank) {
		...
		retv  = db.getBottomURL(rank); // print out given urls.
	}
	else {
